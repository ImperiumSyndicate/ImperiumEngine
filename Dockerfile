...
ENV VIRTUAL_ENV=/opt/imperiumengine-env
ENV PATH=$VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/imperiumengine/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/imperiumengine/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/imperiumengine/ && touch src/imperiumengine/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/imperiumengine/
